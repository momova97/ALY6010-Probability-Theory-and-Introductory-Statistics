...
Hood<- rob %>%
  ...
  dplyr::summarize(NumberOfRobbery = n(),MostOffence = Mode(offence),Division = Mode(Division),Datemean = Mode(datatime),
            Long = mean(Long),Lat = mean(Lat))
head (Hood)
# Now I group by offence
Offence<- rob %>%
  group_by(offence) %>%
  dplyr::summarize(NumberOfRobbery = n(),MostHood = Mode(Neighbourhood),MostHood_ID = Mode(Hood_ID),Division = Mode(Division),Datemean = Mode(datatime),
            Long = mean(Long),Lat = mean(Lat))
head (Offence)
# Now I group by date
Date<- rob %>%
  group_by(year(datatime),month(datatime)) %>%
  dplyr::summarize(NumberOfRobbery = n(),MostOffence = Mode(offence),MostHood = Mode(Neighbourhood),MostHood_ID = Mode(Hood_ID),Division = Mode(Division),
            Long = mean(Long),Lat = mean(Lat))
Date <- Date[-c(1,82),]
